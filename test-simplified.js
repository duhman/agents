...
const index_js_1 = require("./packages/core/src/index.js");
const templates_enhanced_js_1 = require("./packages/prompts/src/templates-enhanced.js");
...
+47 12345678
`;
const masked = (0, index_js_1.maskPII)(testEmailWithPII);
console.log("\nOriginal:");
...
console.log("=".repeat(80));
function testExtraction(email, description) {
    console.log(`\n${description}`);
    ...
    const norwegianCount = norwegianIndicators.filter(word => emailLower.split(/\s+/).includes(word)).length;
...
testExtraction("Hei, jeg skal flytte til Oslo den 15. mars og ønsker å si opp abonnementet mitt.", "Norwegian cancellation with moving");
testExtraction("Hello, I am moving to a new city on March 15 and need to cancel my subscription.", "English cancellation with moving");
testExtraction("Hei, jeg har et spørsmål om faktura.", "Norwegian general inquiry (not cancellation)");
console.log("\n✓ Extraction logic works\n");
console.log("=".repeat(80));
console.log("TEST 3: Draft Generation (Templates)");
console.log("=".repeat(80));
function testDraftGeneration(language, reason, moveDate, description) {
    console.log(`\n${description}`);
    console.log("-".repeat(40));
    const draft = (0, templates_enhanced_js_1.generateDraftEnhanced)({ language, reason, moveDate });
    console.log(`Language: ${language}`);
    console.log(`Reason: ${reason}`);
