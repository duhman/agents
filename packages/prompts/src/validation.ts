...
export function validatePolicyCompliance(
  ...
  const checks = {
    ...
    has_self_service_instructions: draftText.toLowerCase().includes("app"),
  ...
  if (
    !checks.has_self_service_instructions &&
    edgeCase !== "no_app_access" &&
    edgeCase !== "already_canceled"
  ) {
