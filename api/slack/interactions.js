...
async function getRawBody(req) {
    const existing = req[RAW_BODY_SYMBOL];
    if (existing) {
        return existing;
    }
    if (typeof req.body === "string") {
        const buffer = Buffer.from(req.body);
        req[RAW_BODY_SYMBOL] = buffer;
        return buffer;
    }
 ...
 */
function verifySlackSignature(req, requestBody) {
...
function parseSlackPayload(rawBody, contentTypeHeader) {
    if (!rawBody) {
        return null;
    }
...
function isTransientDbError(message) {
    return /CONNECT_TIMEOUT|ETIMEDOUT|ECONNRESET|ECONNABORTED|EAI_AGAIN|ENOTFOUND|Too many connections|connection timed out|Authentication timed out/i.test(message || "");
}
async function withDbRetry(op, ctx) {
    const maxAttempts = 3;
...
async function validateSlackToken(requestId) {
...
async function slackApi(method, body, requestId) {
    const token = process.env.SLACK_BOT_TOKEN;
    if (!token)
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            const canRetry = isAbort || /fetch failed|ECONNRESET|ENOTFOUND|EAI_AGAIN|TLS|ETIMEDOUT/i.test(message);
...
const escapeMrkdwn = (text) => text.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
export function buildEditModalView(params) {
    ...
    if (channelId)
        metadataPayload.channelId = channelId;
    if (messageTs)
        metadataPayload.messageTs = messageTs;
...
export function buildRejectModalView(params) {
    const { ticketId, draftId, channelId, messageTs } = params;
    const metadataPayload = {
        ticketId,
        draftId
...
function safeParseAction(action, requestId, actionId) {
    try {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const requestId = payload?.message?.ts ||
        ...
        if (type === "block_actions") {
            ...
            if (actionId === "approve") {
                ...
                void (async () => {
                    ...
                    try {
                        ...
                        await withDbRetry(() => createHumanReview({
                            ticketId,
                            draftId,
                            decision: "approve",
            ...
            if (actionId === "reject") {
                ...
                void (async () => {
                    ...
                    try {
                        ...
                        void validateSlackToken(requestId).catch((error) => {
            ...
            if (actionId === "edit") {
                try {
                    ...
                    const { ticketId, draftId } = parsed;
                    const draft = await withDbRetry(() => getDraftById(draftId), { requestId, actionId: "edit" });
                    if (!draft || typeof draft.draftText !== "string") {
        ...
        if (type === "view_submission" && payload.view?.callback_id === "reject_modal") {
            const requestId = payload?.view?.id ||
                payload?.trigger_id ||
                String(Date.now());
            let metadata = {};
            try {
            ...
            const rawReason = payload.view?.state?.values?.rejection_reason_block?.rejection_reason?.value ?? "";
            ...
            const trimmedReason = reason.length > MAX_REJECTION_REASON_LENGTH
