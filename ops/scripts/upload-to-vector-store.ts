...
const inputFilePath = join(process.cwd(), 'vector-store-data-2025-10-10.jsonl');
const tempDir = join(process.cwd(), 'temp-vector-files');
...
async function uploadToVectorStore() {
  ...
  try {
  // Read the JSONL data
  const jsonlContent = readFileSync(inputFilePath, 'utf-8');
  const documents: Document[] = jsonlContent
    .trim()
    .split('\n')
