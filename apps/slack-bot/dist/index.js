...
async function queueSlackRetry(params, options = {}) {
    try {
...
export function createSlackRetryProcessor(deps) {
    return async function processRetryQueue() {
        try {
            ...
            for (const pendingItem of itemsToRetry) {
                ...
                const scheduleRetry = async (errorMessage, delayMs) => {
...
export async function postReview(params, options = {}) {
    ...
    const paymentConcerns = Array.isArray(extraction?.payment_concerns)
        ? extraction.payment_concerns
            ...
            .map((value) => escapeMrkdwn(String(value)))
        : [];
...
export async function getSlackRetryQueueStatus() {
    try {
