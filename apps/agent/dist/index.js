...
import { maskPII, envSchema, withRetry, generateRequestId, logInfo, logError } from "@agents/core";
import { extractionSchema, extractionPrompt, systemPolicyEN, generateDraft } from "@agents/prompts";
...
export async function processEmail(params) {
    ...
    try {
        ...
        if (process.env.USE_AGENTS_SDK === "1") {
            const runResult = await run(triageAgent, maskedEmail);
        ...
        if (extraction.is_cancellation) {
            ...
            logInfo("Email processing completed successfully", { ...logContext, ticketId: ticket.id, duration }, {
...
async function extractFields(maskedEmail, logContext) {
    try {
